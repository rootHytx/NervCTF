...
function renderConnectionInfo(connection, parent) {
    ...
    if (connection.ports && Object.keys(connection.ports).length > 0) {
        if (connection.type === "tcp" || connection.type === "nc") {
            ...
            code.textContent = "nc " + connection.host + " " + Object.values(connection.ports).join(", ");
            parent.append(code);
        ...
        if (connection.type === "tcp" || connection.type === "nc") {
            ...
            var code = document.createElement("code");
            code.textContent = "ssh -p " + connection.port + " user@" + connection.host;
            parent.append(code);
    ...
    fetch("/api/v1/containers/info/" + challenge_id, {
        ...
        .then(function (data) {
            ...
            if (data.status === "not_found") {
                alert.textContent = "No active instance. Click 'Fetch Instance' to start.";
                hideUpdateBtns();
                showCreateBtn();
...
(function () {
    ...
    function _makeBtn(label, cls, challengeId, fn) {
        ...
        btn.addEventListener("click", function () { fn(challengeId); });
    ...
    function checkAndInject() {
        ...
        var challengeBody = challengeWindow ? challengeWindow.querySelector(".modal-body") : null;

        if (challengeBody && !document.querySelector(".deployment-actions-injected")) {
            var challengeId = null;
            if (window.challenge && window.challenge.data && window.challenge.data.id) {
                challengeId = window.challenge.data.id;
            }
            if (!challengeId && window.CTFd && window.CTFd._internal && window.CTFd._internal.challenge && window.CTFd._internal.challenge.data) {
            ...
            var createBtn = _makeBtn("Fetch Instance", "btn-primary", challengeId, container_request);
            createBtn.id = "create-chal";
            var extendBtn = _makeBtn("Extend Time", "btn-info", challengeId, container_renew);
            extendBtn.id = "extend-chal";
            var stopBtn = _makeBtn("Terminate", "btn-danger", challengeId, container_stop);
